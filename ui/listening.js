...
window.dexterity.onNative(event=>{
 if(event.type==='listening' && event.active) {
  ...
  document.getElementById('transcript').textContent=cloud?'Speak naturally. Pause when done, or click Done speaking.':'Try “fill this form” or ask about your screen.';
  document.getElementById('done').hidden=!cloud;document.getElementById('done').disabled=event.phase==='starting';
 }
 if(event.type==='voice-transcribing') { document.getElementById('title').textContent='Writing what you said…';document.getElementById('voice-provider').textContent=event.engine+' voice · microphone off';document.getElementById('transcript').textContent='Transcribing your recording. Your request starts next; Cancel stops it.';document.getElementById('done').disabled=true;document.getElementById('voice-countdown').textContent='Microphone off'; }
