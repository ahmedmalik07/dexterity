...
api.onNative(async event=>{
 ...
 if(event.type==='transcript' && !handlingTranscript) {
  ...
  try {
   ...
   const options={mode,remembered:true,companion:true};
   if(event.review!==false && typeof reviewHeard==='function') reviewHeard(text,options);
   else await startTask(text,options);
