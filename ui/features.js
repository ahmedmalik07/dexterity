...
document.querySelector('.window-picker').insertAdjacentHTML('beforebegin','<div class="browser-launch"><div><strong>Filling a web form?</strong><p>Use this browser if your existing browser shows no fields. Sign in to your site here if needed.</p></div><input id="form-url" type="url" placeholder="https://your-form.com" aria-label="Form website address"><button class="secondary" id="open-form-browser">Open form browser ↗</button></div>');
$('open-form-browser').onclick=async()=>{try{await api.openFormBrowser($('form-url').value);toast('Form browser opened. Navigate to your form, then refresh the window list and inspect it.');}catch(e){toast(e.message);}};
...
function renderForm(snapshot) {
 ...
 if(!snapshot.fields.length) $('form-empty').innerHTML='<h3>No supported fields found.</h3><p>For a website, use <strong>Open form browser</strong> above. It enables the accessibility support Dexterity needs. Some custom or administrator-only apps require manual interaction. You can also try the practice form.</p>';
