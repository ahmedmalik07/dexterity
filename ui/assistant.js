...
$('task-teach-next').onclick=()=>startTask('I have done that step. Look at my updated screen, check my progress, and teach me the next step.',{mode:'teach'});
...
api.onNative(event=>{
 ...
 if(event.type==='task-start'){taskId=event.id;taskState(true);$('task-status').textContent='Starting…';}
...
function runHeard(){const text=$('voice-heard-text').value.trim(),options=heardOptions||{};closeHeard();if(!text)return toast('There is nothing to run. Say it again.');startTask(text,options);}
