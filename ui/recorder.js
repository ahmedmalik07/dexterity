...
async function startRecording(options){
 await cancelRecording();const current={id:options.id,chunks:[],cancelled:false,hasSpeech:false};recording=current;
 try{
  ...
  const started=performance.now();current.hardStop=setTimeout(finishRecording,15000);
  ...
  current.timer=setInterval(()=>{document.getElementById('voice-countdown').textContent=Math.max(0,15-Math.floor((performance.now()-started)/1000))+'s maximum · Escape cancels';},200);
