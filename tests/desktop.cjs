...
(async () => {
 ...
 try {
  ...
  await page.locator('#speech-vocabulary').fill('IRIS, NADRA, Islamabad'); await page.locator('#voice-review').uncheck();
  await page.getByRole('button', { name: 'Save preferences' }).click();
  const voicePrefs = await page.evaluate(() => window.dexterity.settings());
  assert.equal(voicePrefs.speechVocabulary, 'IRIS, NADRA, Islamabad'); assert.equal(voicePrefs.voiceReview, false);
