...
Status: 12 September 2026, version 1.6. This is a working Windows prototype. The latest change adds voice review, an eight-action limit, and mandatory approval checks. It does not add continuous observation or a Mac version.
...
3. Use a close microphone or headset. Rehearse one sentence in your own accent. Wait for Listening before speaking, then pause or use **Done speaking**. Edit recognition mistakes before pressing **Run**.
...
The single-file `release-v1.6/Dexterity-1.6.0.exe` was built, but Windows Application Control blocked its launch on this PC. It is not the tested launch path for this presentation. `Start Dexterity.cmd` also runs the source version when the project and installed dependencies are present.
...
### 0:25–1:00 — Voice, correction, and Run
...
3. Pause. Show the recognized text in the editable task box. Say: “Recognition mistakes cannot launch a task. I review the words first.”
4. Edit a word if needed, then press **Run**. Show the answer. No screen capture is needed for this example.
...
2. Keep this task: “Fill this practice form with name Alex Builder and email alex@example.test. Submit the registration after I review it.” Press **Start task**. Typed tasks use the mode-specific start button; voice tasks use **Run** after review.
...
| --- | --- |
| Wrong words or background speech | Edit the transcript before Run. Cancel the recording if it is not your request. Typed input remains available. |
...
The Run gate, eight-action cap, and explicit approval rules are completed requirements. They are not unfinished work to remove for the demo. There is no promise of unrestricted control over every app.

## Evidence and architecture for judges

The latest code update passed 42 automated checks, native Windows self-tests, and voice/form integration tests against both source and the packaged application. Packaged voice testing confirmed zero task requests before Run and use of the edited text afterward. The form test exercised real Windows controls, approval, submission, and visible completion. The eight-action and unchanged-screen tests exercise the runner. Provider responses in these integration tests were mocked; generated speech was used for the voice test. A live run still needs the preflight above.
