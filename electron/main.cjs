...
async function startListening() {
 ...
 try {
  if(settings.speechMode==='router'||settings.speechMode==='gemini' || (settings.speechMode==='auto' && (settings.routerKey||settings.geminiKey))) {
   ...
   listeningWindow.webContents.send('voice:record',{id:session.id,pauseMs:settings.speechPause,engine});
...
app.whenReady().then(() => {
 ...
 coach=createCoach({main,coach:coachWindow,pointer,native,broadcast,busy:()=>!!taskRunner?.run,hidePointer,startListening});
 ...
 native.on('event', event => {
  ...
  if(event.type==='cancel-listening') { stopListening().catch(()=>{}); return; }
...
ipcMain.handle('voice:stop',stopListening);
...
ipcMain.handle('voice:ready',(event,id)=>{
 ...
 session.timer=setTimeout(()=>{if(voiceSession!==session)return;listeningWindow.webContents.send('voice:finish');session.timer=setTimeout(()=>endCloudVoice(session,'Recording stopped at the time limit. Try a shorter request.'),2500);},15000);
...
ipcMain.handle('point', (_, step) => {
 ...
 pointer.setPosition(left, top); pointer.webContents.send('point:data', { x: p.x - left, y: p.y - top, title: String(step.title).slice(0, 65) });
...
ipcMain.handle('task:clear',()=>{if(taskRunner.run)throw new Error('Stop the task first.');taskRunner.history=[];return true;});
