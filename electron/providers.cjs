...
async function gemini(settings, question, image, fetcher, spec) {
 ...
 return { ...guide, provider:'Gemini', model:GEMINI_MODEL };
}
...
async function openrouter(settings,question,image,fetcher=globalThis.fetch,spec){
 ...
 const response=await request(fetcher,'https://openrouter.ai/api/v1/chat/completions',{method:'POST',signal:spec?.signal,headers:{'Content-Type':'application/json',Authorization:'Bearer '+settings.routerKey,'X-Title':'Dexterity'},body:JSON.stringify({model,messages:[{role:'system',content:spec?.instructions||instructions},{role:'user',content}],response_format:{type:'json_schema',json_schema:{name:'dexterity_response',strict:true,schema:spec?.schema||schema}},provider:{require_parameters:true},max_tokens:2600,temperature:0.1,reasoning:{effort:'low'}})},'OpenRouter');
...
module.exports = { DEFAULT_MODEL, GEMINI_MODEL, ROUTER_MODEL, analyzeWithFallback,openrouter };
