...
class TaskRunner {
 ...
 const run={id:require('node:crypto').randomUUID(),goal:input.goal.trim(),mode:input.mode,screen:input.screen!==false,windowId:String(input.windowId||''),remembered:!!input.remembered,companion:input.companion===true,progress:[],controller:new AbortController(),count:0,memories:this.memory?this.memory(input.goal,input.useMemory!==false):[]};
